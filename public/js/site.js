(() => {
  ...
  const removeModal=qs('[data-cart-remove-modal]'),removeForm=qs('[data-cart-remove-form]'),removeId=qs('[data-cart-remove-product-id]'),removeName=qs('[data-cart-remove-product]');
  const closeRemoveModal=()=>{if(removeModal){removeModal.hidden=true;document.body.classList.remove('modal-open')}};
  qsa('[data-cart-remove]').forEach(btn=>btn.addEventListener('click',()=>{if(!removeModal||!removeForm||!removeId)return;removeId.value=btn.dataset.cartRemove;removeName.textContent=btn.dataset.cartRemoveName||'This product';removeModal.hidden=false;document.body.classList.add('modal-open')}));
  qsa('[data-cart-remove-close]').forEach(btn=>btn.addEventListener('click',closeRemoveModal));
  qsa('[data-cart-remove-action]').forEach(btn=>btn.addEventListener('click',()=>{if(!removeForm)return;removeForm.action=btn.dataset.cartRemoveAction;btn.disabled=true;removeForm.submit()}));
